...
#elif  /*#if defined(__MINGW32__) || defined(_MSC_VER)*/
...
#else
    ssize_t res = brlapi__waitForPacket(handle, 0, NULL, 0, TRY_WAIT_FOR_EXPECTED_PACKET);
#endif // !Brl56
...
static void updateSettings(brlapi_connectionSettings_t * s1, const brlapi_connectionSettings_t * s2)
    ...
    brlapi_fileDescriptor BRLAPI_STDCALL brlapi__openConnection(brlapi_handle_t * handle, const brlapi_connectionSettings_t * clientSettings, brlapi_connectionSettings_t * usedSettings)
    {
...
#ifdef _MSC_VER
                char* host = (char*)malloc((hostLength + portLength + 1) * sizeof(*host));
#else
        ...
        if ((len = brlapi__waitForPacket(handle, BRLAPI_PACKET_VERSION, &serverPacket, sizeof(serverPacket), 1,0)) < 0)
            goto outfd;
...
#else
        if ((len = brlapi__waitForPacket(handle, BRLAPI_PACKET_AUTH, &serverPacket, sizeof(serverPacket), WAIT_FOR_EXPECTED_PACKET)) < 0)
            return BRLAPI_INVALID_FILE_DESCRIPTOR;
#endif // !Brl56
...
#else
        res = brlapi__waitForPacket(handle, BRLAPI_PACKET_PACKET, buf, size, TRY_WAIT_FOR_EXPECTED_PACKET);
#endif // !Brl56
        if (res == -3) {
            brlapi_libcerrno = EINTR;
            brlapi_errno = BRLAPI_ERROR_LIBCERR;
...
#else
        res = brlapi__waitForPacket(handle, request, packet, size, WAIT_FOR_EXPECTED_PACKET);
...
#else
            rlen = brlapi__waitForPacket(handle, BRLAPI_PACKET_PARAM_VALUE, reply, sizeof(*reply), 1);
#endif // !Brl56
...
#else
            res = brlapi__waitForPacket(handle, BRLAPI_PACKET_KEY, buf, sizeof(buf), TRY_WAIT_FOR_EXPECTED_PACKET);
#endif // !Brl56
...
#ifndef Brl56
            res = brlapi__waitForPacket(handle, BRLAPI_PACKET_KEY, buf, sizeof(buf), 0, 0);
#else
